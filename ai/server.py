...
import threading
import bluetooth_utils  # Import Wi-Fi utility functions
import email_utils 
...
def monitor_bluetooth_connection(user_email, user_mac_address, stop_event):
    """
    Continuously monitor the Bluetooth connection of a device.
    Send an email if Bluetooth is turned off or if the device disconnects.
    """
    ...
    while not stop_event.is_set():  # Stop monitoring if stop_event is set
        try:
            is_connected = bluetooth_utils.is_device_connected_sync(user_mac_address)

            if not is_connected and previously_connected:
                # Device just disconnected
                subject = "Bluetooth Device Disconnected"
                body = (
                    f"Dear User,\n\nYour Bluetooth device ({user_mac_address}) has been disconnected. "
                    f"If this was unexpected, please check your device.\n\nRegards,\nSystem Log AI Team"
                )
                email_utils.send_email(user_email, subject, body)
                print("Bluetooth is disconnected! Email sent.")
                ...
                print("Bluetooth is reconnected! Resuming monitoring.")
                ...
                print("Bluetooth is connected. Monitoring...")

        except OSError as e:
            # Handle Bluetooth turned off or not available
            if not email_sent and "The device is not ready for use" in str(e):
                subject = "Bluetooth is Turned Off"
                body = (
                    f"Dear User,\n\nBluetooth appears to be turned off on your system. "
                    f"Please enable it to monitor your device ({user_mac_address}).\n\nRegards,\nSystem Log AI Team"
                )
                email_utils.send_email(user_email, subject, body)
                print("Bluetooth is turned off! Email sent.")
                email_sent = True  # Ensure the email is only sent once

        except BleakError as e:
            # Handle other Bleak-related errors
            print(f"BleakError: {e}")

        time.sleep(60)  # Check every 60 seconds

    print(f"Bluetooth monitoring stopped for user: {user_email}")


def start_bluetooth_monitoring(user_email, user_mac_address):
    """
    Start a new thread to monitor the Bluetooth connection of a device.
    """
    ...
    monitor_thread = threading.Thread(target=monitor_bluetooth_connection, args=(user_email, user_mac_address, stop_event))
...
def logs():
    try:
        ...
        user_mac_address = user_data.get("bluetoothAddress")
        ...
        if data['action'] == "login" and incoming_date == datetime.today().date():
            ...
            # Start Bluetooth monitoring for the user
            start_bluetooth_monitoring(user_email, user_mac_address)

        # Check if action is 'logout'
        elif data['action'] == "logout":
            # Stop Bluetooth monitoring for the user
            if user_email in monitoring_threads:
                monitoring_threads[user_email]["stop_event"].set()
                monitoring_threads[user_email]["thread"].join()
        ...
        is_connected = bluetooth_utils.is_device_connected_sync(user_mac_address)
        details = {
            "bluetoothAddress": user_mac_address,
            "bluetoothConnected": is_connected,
        }
