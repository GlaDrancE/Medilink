...
export default function DoctorSettings() {
  ...
  const loadDoctorProfile = async () => {
    try {
      ...
      // Remove the token check - allow API calls without token for development
      ...
      if (response.ok) {
        ...
        // Use fallback profile for development
        const mockProfile = {
          id: 'dev-user-123',
          name: 'Dr. John Doe',
      ...
      // Create a basic profile for development
      const fallbackProfile = {
        id: 'dev-user-123',
        name: 'Dr. Development User',
        email: 'dev@example.com',
        phone: '1234567890',
        hospital: 'Dev Hospital',
        address: 'Dev Address',
  ...
  const handleSave = async () => {
    try {
      ...
      if (!profile.id) {
        alert('Unable to save: Profile ID is missing. Please refresh the page and try again.');
        return;
      }
  ...
  // Add better error handling for missing profile data
  if (isLoading) {
    return (
